...
object Consumer {
  def default[F[_]](conf: EventsConfig)(implicit eff: Effect[F]): Consumer[F] =
    new KafkaFs2Consumer(conf)
}

class KafkaFs2Consumer[F[_]](conf: EventsConfig)(implicit eff: Effect[F]) extends Consumer[F] {
